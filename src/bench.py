...
class Bencher:
    ...
    def estBenchTime(self):
        ...
        count = self.original
        runtime = count * (self.config.checkTime +
                           self.config.absolute * self.config.repetitions)
        print(INFORMATIONAL +
              "WORST CASE time to complete benchmark is " + prettyTime(runtime))

    def getValidProtocols(self):
        # Given a list of protocols, check well-formedness of each
        validProtocols = list()
        start = time()
        for p in tqdm(self.uniqueProtocols, leave=False,
                      smoothing=0.0, desc="Well Formedness Checks"):
        ...
        print(
            INFORMATIONAL +
            "Finished well-formedness checks in " +
            prettyTime(td))
    ...
    def benchProtocol(self, protocol_path):
        ...
        for i in range(0, config.repetitions):
            ...
            output = self.tamarin.getResults(
                protocol_path, diff, config.absolute)
            if "TIMEOUT" in output.lemmas:
                tqdm.write(BENCH_TIMEOUT +
    ...
    def performBenchmark(self):
        ...
        start = time()
        for p in tqdm(protocols, leave=False, smoothing=0.0,
                      desc="Benchmarking protocols"):
            output.write(resultToString(self.benchProtocol(p)) + "\n")
        td = time() - start
        print(INFORMATIONAL + "Finished benchmarking in " +
              prettyTime(td) + " seconds")
    ...
    def printSummary(self):
        ...
        if self.failed > 0:
            print(TERMINAL.bold(TERMINAL.red(
                "FAILED BENCHMARK: " + str(self.failed))))
        if self.nolemmas > 0:
            print(TERMINAL.bold(TERMINAL.yellow(
                "NO LEMMAS: " + str(self.nolemmas))))
        if self.original - self.failed - self.nolemmas > 0:
            print(TERMINAL.bold(TERMINAL.green("SUCCESSFUL: " +
                                               str(self.original - self.failed - self.check - self.nolemmas))))
        print("=====================================")
        print("=====================================")
        print("=====================================")
