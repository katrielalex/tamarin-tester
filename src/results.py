...
def compareResults(output, bench):
    # This function compares two Results objects returns a message describing
    # any discrepancies
    message = ""
    # For  each lemma
    for i in range(0, len(output.lemmas)):
        if output.lemmas[i][0] != bench.lemmas[i][0]:
            # This should literally never happen, but a sanity check never hurt
            # the sane
        ...
        if output.lemmas[i][1] != bench.lemmas[i][1]:
            message += INCORRECT + output.lemmas[i][0] + " was: " + \
                bench.lemmas[i][1] + " now: " + output.lemmas[i][1] + "\n"
        # Check for a change in step count
        if testSteps > benchSteps:
            message += STEPSIZE_INC + \
                output.lemmas[i][0] + " was: " + \
                str(benchSteps) + " now: " + str(testSteps) + "\n"
        if testSteps < benchSteps:
            message += STEPSIZE_DEC + \
                output.lemmas[i][0] + " was: " + \
                str(benchSteps) + " now: " + str(testSteps) + "\n"
    return message
...
def resultToString(res):
    ...
    return res.fileHash + "|" + \
        str(res.diff) + "|" + str(res.lemmas) + "|" + \
        str(res.avgTime) + "|" + str(res.flags)
