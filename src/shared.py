...
"""
shared.py: common utilities for tamarin-tester
"""
...
log = logging.getLogger("tamarin-tester")
...
class Settings:
    def __init__(self, args):
        ...
        # Max multiples of benchmark time to wait for
        self.contingency = args.contingency
        # How many samples to take for benchmark average
...
def runAsDiff(tamarin, path, t):
    if validNormProtocol(tamarin, path, t) != 1 and validDiffProtocol(
            tamarin, path, t) == 1:
        return 1
    else:
        return 0
...
tamarin-tester is a tool for testing the correctness of tamarin-prover builds by comparing their
output to known-good builds. For a more comprehensive overview, consult the README distributed with
this program. In general, you may run tests against benchmark files or generate these benchmark
files yourself. Authored by Dennis Jackson, Computer Science Dept, University of Oxford.""".replace('\n', ''))
