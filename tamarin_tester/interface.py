...
class Tamarin:
    ...
    def getResults(self, protocol_path, diff, timeout):
        try:
            with open(os_devnull, 'w') as devnull:
                ...
                command = marshalCommand(self.path, getFlags(
                    self.flags, 1, diff, protFlags), protocol_path)
                ...
                return outputToResults(
                    strOutput, protocol_path, diff, 0.0, protFlags)
    ...
    def isWellFormed(self, path, diff, timeout):
        ...
        try:
            with open(os_devnull, 'w') as devnull:
                command = marshalCommand(self.path, getFlags(
                    self.flags, 0, diff, extractFlags(path)), path)
