...
class Tester:
    def __init__(self, config):
        ...
        if config.absolute == 0.0:
            ...
            if self.config.verbose:
                print(INFORMATIONAL + "Loaded default max proof time from file " +
                      prettyTime(config.checkTime))
        elif self.config.verbose:
            print(INFORMATIONAL + "Max Proof Time (from argument): " +
                  prettyTime(config.absolute))
        ...
        if config.checkTime == 0.0:
            ...
            if self.config.verbose:
                print(INFORMATIONAL + "Used default max check time: (min(prooftime, maxFileTime)) " +
                      prettyTime(config.checkTime))
        elif self.config.verbose:
            print(INFORMATIONAL + "Max Check Time (from argument): " +
                  prettyTime(config.checkTime))
    ...
    def checkOvertime(self):
        ...
        if goingOvertime > 0:
            if not self.config.removeOvertime:
                print(WARNING + str(goingOvertime) + " protocol(s) are not expected to terminate proving because the max proof time " +
                      prettyTime(self.config.absolute) + " is less than their expected run time (Highest seen: " + prettyTime(maxSeen) + ")")
            return 1
        else:
            return 0
    ...
    def estTestTime(self):
        ...
        print(INFORMATIONAL + "Expected Test runtime is at least " +
    ...
    def performTest(self):
        ...
        start = time()
        for b in tqdm(sorted(self.benchmarks, key=lambda bench: bench.avgTime),
                      smoothing=1.0, leave=False, desc="Testing against benchmarks"):
            if self.config.failfast and self.failures > 0:
                print(
                    ERROR +
                    " a test failed and failfast is enabled. Quitting...")
                exit(1)
            if b.fileHash not in hashes or "TIMEOUT" in b.lemmas:
                # We can ignore this benchmark as either we have no data or its
                # not in our test input
                continue
            hashes.remove(b.fileHash)
            # NoLemmas means that the benchmark had nothing to prove and hence
            # we cannot test here
            if "NOLEMMAS" in b.lemmas:
                self.nolemmas += 1
                tqdm.write(
                    NO_LEMMAS + self.hashToPath[b.fileHash][len(config.protocols):])
                continue
            # Here we check for well formedness
            if (not b.diff and validNormProtocol(self.tamarin, self.hashToPath[b.fileHash], self.config.checkTime)) or (
                    b.diff and validDiffProtocol(self.tamarin, self.hashToPath[b.fileHash], self.config.checkTime)):
                tqdm.write(self.testProtocol(
                    self.hashToPath[b.fileHash], b), end="")
            else:
                # Both test and benchmark versions should agree on well
                # formedness
                tqdm.write(FAILED + self.hashToPath[b.fileHash][len(
    ...
    def printSummary(self):
        ...
        if self.missing != 0:
            print(TERMINAL.bold(TERMINAL.yellow(
                "NO BENCHMARK: " + str(self.missing))))
        if self.removedOvertime != 0:
            print(TERMINAL.bold(TERMINAL.yellow(
                "OVERTIME: " + str(self.removedOvertime))))
        if self.nolemmas != 0:
        ...
        if self.failures > 0:
            print("=====================================")
            print("=============== " +
                  TERMINAL.red(TERMINAL.bold("FAIL")) + " ================")
            print("=====================================")
        elif self.warning + self.missing + self.nolemmas + self.removedOvertime > 0:
            print("=====================================")
            print("=============== " +
                  TERMINAL.yellow(TERMINAL.bold("WARN")) + " ================")
            print("=====================================")
        else:
            print("=====================================")
            print("=============== " +
                  TERMINAL.green(TERMINAL.bold("PASS")) + " ================")
            print("=====================================")
    ...
    def testProtocol(self, protocol_path, bench):
        ...
        if "INCORRECT" in message:
            self.failures += 1
            return FAILED + \
                protocol_path[len(config.protocols):] + "\n" + message
